...
category_ids = [
    2204, 1306, 1308, 1309, 1313, 2403, 2404, 2406, 2502, 2700, 2701, 2704, 2705, 2706, 2712, 2713, 2716, 2720, 2725, 2728, 2730, 2735, 2736, 2737, 2738, 2739, 2746, 2800, 3002, 3003, 3004, 3005
]
...
for category_id in category_ids:
    ...
    for _, row in df.iterrows():
        name = row[journal_col]
        sjr_raw = str(row[impact_col])
        sjr_num = sjr_raw.split()[0] if sjr_raw else ''
        try:
            sjr = float(sjr_num)
        except Exception:
            continue
